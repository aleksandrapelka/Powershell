...
$pobierzDane = 'https://api.covid19api.com/summary'
$a = Invoke-WebRequest $pobierzDane | ConvertFrom-Json 
$a.Countries | Select-Object Country, NewConfirmed, TotalConfirmed, NewDeaths, TotalDeaths, NewRecovered, TotalRecovered, Date | Format-Table
...
#{
 ...
 #    $pobranaData[$i].Remove(19,5) -replace "T", " " #obciecie koncowki i zamiana T na spacje
...
$data = for($i = 0; $i -lt $pobranaData.Count; $i++)
{
    ...
    elseif($c.Days -eq 0 -and $c.Hours -eq 0)
    {
        $minuta = "$minuta min "
        $g = $minuta
...
$nowaTabela = for($i = 0; $i -lt $nowa.Count; ++$i)
{
    ...
    [PSCustomObject]@{
             ...
             Kraj = $nowa[$i].Country
             NoweZakażenia = $nowa[$i].NewConfirmed
             WszystkieZakażenia = $nowa[$i].TotalConfirmed
             NoweZgony = $nowa[$i].NewDeaths
             WszystkieZgony = $nowa[$i].TotalDeaths
             NoweOzdrowienia = $nowa[$i].NewRecovered
             WszytkieOzdrowienia = $nowa[$i].TotalRecovered
             Data = $data[$i]
